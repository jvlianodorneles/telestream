...
// Generates standard 2D boolean matrix for QML Canvas or SVG rendering
...
var QRCode = (function () {
  ...
  // Reed-Solomon Error Correction Tables for all 40 QR Versions
  var RS_BLOCK_TABLE = [
    // 1-10
  ...
  // Alignment pattern locations for all 40 QR Versions
  ...
  QRCodeModel.prototype = {
    ...
    make: function () {
      if (this.typeNumber < 1) {
        ...
        if (typeNumber > 40) typeNumber = 40;
        this.typeNumber = typeNumber;
      }
  ...
  /**
   * Generates a 2D boolean matrix for a given text.
   * @param {string} text - URL or text string to encode.
   * @param {string} errorLevel - "L", "M", "Q", or "H" (defaults to "M").
   * @returns {Array<Array<boolean>>} 2D boolean array where true = dark/black module.
   */
  function generateMatrix(text, errorLevel) {
    if (!text || text.length === 0) return [];
    var ecLevel = QRErrorCorrectLevel.M;
    if (errorLevel === "L") ecLevel = QRErrorCorrectLevel.L;
    else if (errorLevel === "Q") ecLevel = QRErrorCorrectLevel.Q;
    else if (errorLevel === "H") ecLevel = QRErrorCorrectLevel.H;
